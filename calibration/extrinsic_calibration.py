...
s_imgs = utils.find_images_in_dir(args.data_dir + '/images/%s/static'%args.cam)
print 'read %d images from static camera'%len(s_imgs)
w_imgs = utils.find_images_in_dir(args.data_dir + '/images/%s/wrist'%args.cam)
print 'read %d images from wrist camera'%len(w_imgs)

if args.cam == 'both':
    s_imgs, s_points = utils.find_imagepoints_from_images(s_imgs, args.w, args.h, square=True, show_imgs=args.show_imgs)
    w_imgs, w_points = utils.find_imagepoints_from_images(w_imgs, args.w, args.h, square=True, show_imgs=args.show_imgs, rotate_angle=90)
...
print 'found patterns in %d images'%len(img_indices)
for i in img_indices:
    with open(args.data_dir + '/poses/%s/%d.yaml'%(args.cam, i), 'rb') as f:
...
if args.cam=='both':
    ...
    #w_C2I = np.matmul(np.array([[0, -1, 480], [1, 0, 0], [0, 0, 1]]), w_C2I)
...
w_C2W_trans_0 = np.array([-0., -0.09, 0.])
...
s_C2B_ = None
def w_reprojected_to_s(x):
    if len(x) == 7:
        s_C2B = s_C2B_
        w_C2W = utils.quat_trans_to_rigid(x[0:4], x[4:7])
    else:
        s_C2B = utils.quat_trans_to_rigid(x[0:4], x[4:7])               # 4 * 4
    ...
    w_p_C = np.matmul(w_I2C, w_p_I)                                 # b * n * 3 * 1
    w_p_C = np.concatenate((w_p_C, np.ones([b, n, 1, 1])), axis=2)  # b * n * 4 * 1
    w_p_B = np.matmul(np.expand_dims(w_C2B, 1), w_p_C)              # b * n * 4 * 1

    o_C = [[0], [0], [0], [1]]                              # 4 * 1
    o_B = np.expand_dims(np.matmul(w_C2B, o_C), 1)          # b * 1 * 4 * 1
    
    p = o_B - (o_B[:, :, 2:3, :] / (o_B[:, :, 2:3, :] - w_p_B[:, :, 2:3, :])) * (o_B - w_p_B)   # b * n * 4 * 1
    p = np.matmul(np.linalg.inv(s_C2B), p)                  # b * n * 4 * 1
    p = p[:, :, :3, :]                                      # b * n * 3 * 1
    p = np.matmul(s_C2I, p)                                 # b * n * 3 * 1
    p = p / np.expand_dims(p[:, :, 2, :], axis=2)
...
def repr_loss(x):
    # x : concatenation of s_C2B_quat, s_C2B_trans, w_C2W_quat, w_C2W_trans. shape : (14)
    global step
    step += 1
    
    p = w_reprojected_to_s(x)
...
if args.cam == 'both':
    x = minimize(repr_loss, np.concatenate([s_C2B_quat_0, s_C2B_trans_0, w_C2W_quat_0, w_C2W_trans_0]), method = args.method, options={'xtol':1e-9, 'maxiter' : 100000, 'maxfev' : 640000, 'disp' : True}).x
    s_C2B_ = utils.quat_trans_to_rigid(x[0:4], x[4:7])
    x_ = minimize(repr_loss, x[7:14], method= args.method, options={'xtol':1e-9, 'maxiter' : 100000, 'maxfev' : 640000, 'disp' : True}).x
    x = np.concatenate((x[0:7], x_))

    p = w_reprojected_to_s(x)
    for i in range(len(w_imgs)):
        s_img_w_reprojected = np.copy(s_imgs[i])
        for j in range(n):
            s_img_w_reprojected = cv2.circle(s_img_w_reprojected, (int(p[i][j][0][0]), int(p[i][j][1][0])), 3, (0, 0, 255), -1)
        cv2.imshow('s_img_w_reprojected', s_img_w_reprojected)
        cv2.waitKey(0)

elif args.cam == 'wrist':
    x = minimize(wrist_loss, np.concatenate([w_C2W_quat_0, w_C2W_trans_0, T2B_quat_0, T2B_trans_0]), method = args.method, options={'xtol':1e-5, 'maxiter' : 100000, 'maxfev' : 640000, 'disp' : True}).x
    p = w_reprojected(x)
    for i in range(len(w_imgs)):
        w_img_reprojected = np.copy(w_imgs[i])
        for j in range(n):
            w_img_reprojected = cv2.circle(w_img_reprojected, (int(p[i][j][0][0]), int(p[i][j][1][0])), 3, (0, 0, 255), -1)
        cv2.imshow('w_img_reprojected', w_img_reprojected)
        cv2.waitKey(0)

elif args.cam == 'static':
    x = minimize(static_loss, np.concatenate([s_C2B_quat_0, s_C2B_trans_0, w_T2W_quat_0, w_T2W_trans_0]), method = args.method, options={'xtol':1e-5, 'maxiter' : 10000, 'maxfev' : 640000, 'disp' : True}).x
