...
def pose1(img_color, contours, cx,cy,angle) : #block4_2
...
def pose2(img_color, contours, cx,cy,angle) : #block4_2
     for cnt in contours:
        area = cv2.contourArea(cnt)
...
def pose3(img_color, contours, cx,cy,angle) : #block4_2
     for cnt in contours:
        area = cv2.contourArea(cnt)
        if((area >2000)&(area<100400)) :
...
def pose4(img_color, contours, cx,cy,angle) : #block4_2
     for cnt in contours:
        area = cv2.contourArea(cnt)
        if((area >2000)&(area<100400)) :
...
def pose5(img_color, contours, cx,cy,angle) : #block4_2
     for cnt in contours:
        area = cv2.contourArea(cnt)
        if((area >2000)&(area<100400)) :
...
def pose6(img_color, contours, cx,cy,angle) : #block4_2
     for cnt in contours:
        area = cv2.contourArea(cnt)
        if((area >2000)&(area<100400)) :
...
def pose7(img_color, contours, cx,cy,angle) : #block4_2
     for cnt in contours:
        area = cv2.contourArea(cnt)
        if((area >2000)&(area<100400)) :
...
def pose8(img_color, contours, cx,cy,angle) : #block4_2
     for cnt in contours:
        area = cv2.contourArea(cnt)
        if((area >2000)&(area<100400)) :
...
def find_grip_points(img_fname):
    deeplab = DeeplabModule()
    out = deeplab.run(img_fname)
...
# mask = np.all(res == (128, 128, 128), axis=-1).astype(np.uint8)*255
    ...
    centers = []
    for i in range(7):
        ct, angle = find_center(blocks_contours[i])
        print ct, angle
        blocks_cts[i] = ct
...
#     print(temp_img.shape)
    ...
    while i <7:
        ...
        if pl[i] == 1:
            center,rot_point= pose1(img_color, contours, cx,cy,b[i])
            print(b[i])
            i+=1
            
        elif pl[i] == 2:
            center,rot_point= pose2(img_color, contours, cx,cy,b[i])
            ...
            center,rot_point= pose4(img_color, contours, cx,cy,b[i])
            
            i+=1
        elif pl[i] == 5:
            center,rot_point= pose5(img_color, contours, cx,cy,b[i])
            ...
            center,rot_point= pose8(img_color, contours, cx,cy,b[i])
            i+=1
        rot_point_tuple = (rot_point[0], rot_point[1])
        cv2.circle(img_overlayed, rot_point_tuple, 1,(0,125, 255), 3)
